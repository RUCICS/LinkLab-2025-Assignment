...
编译器会为每个文件生成一个目标文件。`foo.fle`的内容可能是这样的：
...
注意`main.fle`中有个`❓`标记——这是一个待解决的引用。代码想要访问`message`数组，但现在它还不知道这个数组会被放在内存的哪里。这正是链接器要解决的问题。
...
```
0x400000: [main.fle的.text节]  (32字节)
0x400020: [foo.fle的.rodata节] (14字节)
```

这样，`message`符号原本在`foo.fle`的`.rodata`节偏移0处，现在就对应内存地址`0x400020`。当然，实际实现时你还需要考虑其他的节，比如`.data`节、`.bss`节等，但核心思路是一样的。

**第二个问题：如何找到每个符号的定义？**

这就是符号解析的过程。在上面的例子中，`main.fle`使用了`message`符号，而`foo.fle`定义了它。链接器需要建立一个全局的视角，知道每个符号在哪里定义、最终位于什么地址。

一种自然的做法是维护一个全局符号表。当你处理每个目标文件时，检查它定义了哪些符号，记录下这些符号的最终地址。这样，当遇到未定义的符号引用时，你就可以查表找到它的地址。

**第三个问题：如何填补那些空缺的地址引用？**

这就是重定位。回到我们的例子，`main.fle`中有一行：
...
完成上述三步后，我们得到了一块包含完整机器码的内存映像。最后一步是将它包装成可执行文件的格式。

在FLE格式中，可执行文件需要包含程序头（program headers），描述程序运行时的内存布局。对于这个任务，我们可以使用一种简单的策略：把所有内容放在一个段中。创建一个程序头描述这个段的位置、大小和权限：
...
> [!TIP]
> 你可能注意到我们暂时把所有内容放在一个段中，并赋予了读、写、执行的全部权限。这不是最终的方案——真实的系统中，代码和数据应该分开存放，并设置不同的权限。但在这个阶段，我们先专注于让程序运行起来。你会在后续任务中逐步完善这个布局。
>
