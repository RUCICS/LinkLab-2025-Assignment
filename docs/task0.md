...
会生成一个`test.fle`文件。让我们看看它的内容。

## FLE格式文件的结构

FLE格式使用JSON来表示目标文件的结构，这意味着你可以用任何文本编辑器打开它，直接看到里面的内容。这是`test.fle`的样子：
...
当你查看`FLEObject`的定义时，可能会对两个字段感到困惑：`shdrs`（section headers，节头）和`phdrs`（program headers，程序头）。它们看起来都在描述程序的结构，为什么需要两套描述符？

这个问题的答案涉及到文件格式设计的一个核心洞察：链接器和加载器关心的是不同的事情。

**链接器关心的是内容的细分**。当链接器读取一个目标文件时，它需要知道"这个文件包含哪些部分，每个部分是什么，它们如何组合"。代码在哪里？只读数据在哪里？可读写数据在哪里？未初始化的数据在哪里？这些信息通过节头来描述。每个节代表一类内容，有明确的语义——`.text`是代码，`.rodata`是常量，`.data`是初始化的变量。

节头提供了一个细粒度的视图。一个目标文件可能有十几个甚至几十个节：`.text`、`.text.startup`、`.text.unlikely`、`.rodata`、`.rodata.str1.1`、`.data`、`.data.rel.local`、`.bss`等等。每个节可能只有几十或几百字节。这种细分便于链接器进行精确的控制——比如把所有`.text`相关的节合并到一起，把所有`.rodata`相关的节合并到一起。

**加载器关心的是如何映射到内存**。当操作系统加载一个程序时，它不关心代码和数据的逻辑分类，它关心的是"哪些内容应该被加载到内存的什么位置，应该有什么权限"。这些信息通过程序头来描述。每个程序头描述一个段（segment），指定了这个段在文件中的位置、应该被加载到内存的哪个地址、占用多大空间、以及访问权限（可读、可写、可执行）。

程序头提供了一个粗粒度的视图。一个可执行文件通常只有几个段：一个包含所有代码的可执行段，一个包含所有只读数据的只读段，一个包含所有可读写数据的可读写段。操作系统以段为单位进行内存映射和权限设置——每个段对应虚拟地址空间中的一块连续区域。

让我们用一个具体的例子来说明。假设链接器处理了多个目标文件，合并后得到：

- `.text`节：20KB的代码
- `.text.startup`节：4KB的初始化代码  
- `.rodata`节：8KB的字符串常量
- `.rodata.str1.1`节：2KB的对齐字符串
- `.data`节：4KB的全局变量
- `.bss`节：16KB的未初始化变量

链接器会为这六个节各生成一个节头，因为它们在逻辑上是不同的内容。但对于加载器，链接器可能只生成三个程序头：

1. 第一个程序头描述代码段，包含`.text`和`.text.startup`两个节，总共24KB，权限是可读可执行。
2. 第二个程序头描述只读数据段，包含`.rodata`和`.rodata.str1.1`两个节，总共10KB，权限是只读。
3. 第三个程序头描述可读写数据段，包含`.data`和`.bss`两个节，总共20KB，权限是可读可写。

这样，链接器保留了所有细节信息（六个节），但告诉加载器它只需要关心三件事（三个段）。节头是为了链接时的精确控制，程序头是为了运行时的高效加载。
...
理解了这个区别后，你会明白为什么目标文件（`.obj`类型）有节头但没有程序头——因为目标文件不需要被直接加载运行，它只是链接器的输入。相反，可执行文件（`.exe`类型）既有节头也有程序头——节头是历史遗留和调试用途（技术上可以省略），程序头是加载器的必需信息。

在实现链接器时，你会发现自己同时操作这两个层面。在合并节、解析符号、处理重定位时，你工作在节这个层面；在规划内存布局、设置权限、生成最终输出时，你工作在段这个层面。链接器的一个重要职责就是在这两个视角之间建立正确的映射关系。
